...
char r='0';		
float stor=0.0;
int spring_counter=0,enter=0,robot_score=0,enemy_score=0,roww=0,coll=0,evencount=0;
int keep=0,win=0;
...
int counter=0;
using namespace std;
float _angle = 0,dv1=0.5,deltax=0.0,deltaz=0.0,deltay=0.0,ang=0;
...
int fl=0,jump=0,smp=0;
int dir=0,fall_counter=0,spot=0;
float co=0,y_fac=-0.05;	
int fire_no=rand()%64,fire_flag=0;
int block_count=0;
float yy=0.0,vely=0.0,swamp=0.0;
int ran=0;
int factor=0,tower=0,first=0,tower_view=0,first_person_view=0,projectile_front=0,ck_flag=0,third=0;
int cl=0,cv=0,level=1;
float fx,fy,fz,rightx,righty,rightz,ydec=0.0,leftx,lefty,leftz,middlex,middley,middlez,upx,upy,upz,downx,downy,downz,cornerx,cornery,cornerz,forwardx,forwardy,forwardz;
float v4=rand()%10;// missing
/*float v4=rand()%10;// missing
float fx=cx+(v4/20),fy=4+(v4/20),fz=cz-(v4/20);*/
GLuint _textureId[8]; //The id of the texture
GLuint textureId[8];
...
GLuint loadTexture7(Image* image) {
	glGenTextures(1, &textureId[6]); //Make room for our texture
	glBindTexture(GL_TEXTURE_2D, textureId[6]); //Tell OpenGL which texture to edit
	//Map the image to the texture
	glTexImage2D(GL_TEXTURE_2D,                //Always GL_TEXTURE_2D
			0,                            //0 for now
			GL_RGB,                       //Format OpenGL uses for image
			...
			//as unsigned numbers
			image->pixels);               //The actual pixel data
	return textureId[6]; //Returns the id of the texture
}
GLuint loadTexture8(Image* image) {
	glGenTextures(1, &textureId[7]); //Make room for our texture
	glBindTexture(GL_TEXTURE_2D, textureId[7]); //Tell OpenGL which texture to edit
	//Map the image to the texture
	glTexImage2D(GL_TEXTURE_2D,                //Always GL_TEXTURE_2D
			0,                            //0 for now
			GL_RGB,                       //Format OpenGL uses for image
			...
			//as unsigned numbers
			image->pixels);               //The actual pixel data
	return textureId[7]; //Returns the id of the texture
}
...
//	float v4=rand()%10;// missing
		...
		cornerx=5.2;
		cornery=3+(v4/20);
		cornerz=cz-(v4/20)-6.8;
		forwardx=6.2;
		forwardy=3+(v4/20);
		forwardz=cz-(v4/20)-10.0;
...
void check_teleportation_tile(float xxx,float yyy,float zzz)
{
	for(int i=0;i<8;i++)
	{
		for(int j=0;j<8;j++)
		{	
			if(xxx>=tile[0][2].x_cor && xxx<=tile[0][2].x_cor+1.5)
			{
				if(zzz>=tile[0][2].z_cor-1.5 && zzz<=tile[0][2].z_cor)
				{
						//tile[i][j].robo=1;
						cz=-7.0;
						cx=3.0;
...
int check_swampy_tile(float xxx,float yyy,float zzz)
{
	for(int i=0;i<8;i++)
	{
		for(int j=0;j<8;j++)
		{	
			if(xxx>=tile[0][3].x_cor && xxx<=tile[0][3].x_cor+1.5)
			{
				if(zzz>=tile[0][3].z_cor-1.5 && zzz<=tile[0][3].z_cor)
				{
					return 1;
					...
					glVertex3f(0.0f,0.0f-ydec,0.0f);
					glVertex3f(1.50f,0.0f-ydec,0.0f);
					glVertex3f(1.5f,0.3f-ydec,0.0f);
					glVertex3f(0.0f,0.3f-ydec,0.0f);
					glEnd();
					glBegin(GL_QUADS);
					// left face
					glColor3f(0.0f,0.5f,0.0f); //yellow
					glVertex3f(0.0f,0.0f-ydec,0.0f);
					glVertex3f(0.0f,0.0f-ydec,-1.5f);
					glVertex3f(0.0f,0.3f-ydec,-1.5f);
					glVertex3f(0.0f,0.3f-ydec,0.0f);
					glEnd();
					glBegin(GL_QUADS);
					// right
					glColor3f(0.0f,0.5f,0.0f); //green
					glVertex3f(1.5f,0.0f-ydec,0.0f);
					glVertex3f(1.5f,0.0f-ydec,-1.5f);
					glVertex3f(1.5f,0.3f-ydec,-1.5f);
					glVertex3f(1.5f,0.3f-ydec,0.0f);
					glEnd();
					glBegin(GL_QUADS);
					// back face
					glVertex3f(0.0f,0.0f-ydec,-1.5f);
					glVertex3f(1.50f,0.0f-ydec,-1.5f);
					glVertex3f(1.5f,0.3f-ydec,-1.5f);
					glVertex3f(0.0f,0.3f-ydec,-1.5f);
					glEnd(); 
					...
					glVertex3f(1.50f,0.3f-ydec,0.0f);
...
{	
	...
	glColor3f(0.0f,0.5f,0.0f); //red
	glTranslatef(0.0f,0.0f,0.0f);
	glVertex3f(0.0f,0.0f-ydec-swamp,0.0f);
	glVertex3f(1.50f,0.0f-ydec-swamp,0.0f);
	glVertex3f(1.5f,0.3f+yy-ydec-swamp,0.0f);
	glVertex3f(0.0f,0.3f+yy-ydec-swamp,0.0f);
	glEnd();
	glBegin(GL_QUADS);
	// left face
	glColor3f(0.0f,0.5f,0.0f); //yellow
	glVertex3f(0.0f,0.0f-ydec-swamp,0.0f);
	glVertex3f(0.0f,0.0f-ydec-swamp,-1.5f);
	glVertex3f(0.0f,0.3f+yy-ydec-swamp,-1.5f);
	glVertex3f(0.0f,0.3f+yy-ydec-swamp,0.0f);
	glEnd();
	glBegin(GL_QUADS);
	// right
	glColor3f(0.0f,0.5f,0.0f); //green
	glVertex3f(1.5f,0.0f-ydec-swamp,0.0f);
	glVertex3f(1.5f,0.0f-ydec-swamp,-1.5f);
	glVertex3f(1.5f,0.3f+yy-ydec-swamp,-1.5f);
	glVertex3f(1.5f,0.3f+yy-ydec-swamp,0.0f);
	glEnd();
	glBegin(GL_QUADS);
        // back face
	glVertex3f(0.0f,0.0f-ydec-swamp,-1.5f);
	glVertex3f(1.50f,0.0f-ydec-swamp,-1.5f);
	glVertex3f(1.5f,0.3f+yy-ydec-swamp,-1.5f);
	glVertex3f(0.0f,0.3f+yy-ydec-swamp,-1.5f);
	glEnd();
/*
...
//	glColor3f(0.05f,0.05f,0.05f);
	...
	glVertex3f(0.0f,0.3f+yy-ydec-swamp,0.0f);
	glTexCoord2f(1.0f, 0.0f);
	glVertex3f(1.50f,0.3f+yy-ydec-swamp,0.0f);
	glTexCoord2f(1.0f, 1.0f);
	glVertex3f(1.5f,0.3f+yy-ydec-swamp,-1.5f);
	glTexCoord2f(0.0f, 1.0f);
	glVertex3f(0.0f,0.3f+yy-ydec-swamp,-1.5f);
...
{	
	...
	glVertex3f(0.0f,0.0f-ydec-swamp,0.0f);
	glVertex3f(1.50f,0.0f-ydec-swamp,0.0f);
	glVertex3f(1.5f,0.3f-ydec-swamp,0.0f);
	glVertex3f(0.0f,0.3f-ydec-swamp,0.0f);
	glEnd();
	glBegin(GL_QUADS);
	glVertex3f(0.0f,0.0f-ydec-swamp,0.0f);
	glVertex3f(0.0f,0.0f-ydec-swamp,-1.5f);
	glVertex3f(0.0f,0.3f-ydec-swamp,0.0f);
	glVertex3f(0.0f,0.3f-ydec-swamp,-1.5f);
	glEnd();
	glBegin(GL_QUADS);
	glVertex3f(1.5f,0.0f-ydec-swamp,0.0f);
	glVertex3f(1.5f,0.0f-ydec-swamp,-1.5f);
	glVertex3f(1.5f,0.3f-ydec-swamp,-1.5f);
	glVertex3f(1.5f,0.3f-ydec-swamp,0.0f);
	glEnd();
	glBegin(GL_QUADS);
	glVertex3f(0.0f,0.0f-ydec-swamp,-1.5f);
...
/*
	...
	glVertex3f(0.0f,0.3f-ydec-swamp,0.0f);
	glTexCoord2f(1.0f, 0.0f);
	glVertex3f(1.50f,0.3f-ydec-swamp,0.0f);
	glTexCoord2f(1.0f, 1.0f);
	glVertex3f(1.5f,0.3f-ydec-swamp,-1.5f);
	glTexCoord2f(0.0f, 1.0f);
	glVertex3f(0.0f,0.3f-ydec-swamp,-1.5f);
...
//	glEnable(GL_LIGHT0);
	...
	delete image6;
	Image* image7 = loadBMP("sc.bmp");
	_textureId[6] = loadTexture7(image7);
	delete image7;
	Image* image8 = loadBMP("you_win.bmp");
	_textureId[7] = loadTexture8(image8);
	delete image8;
...
void check_collision_with_ball(float xxx,float yyy,float zzz)
{
	...
	if(xxx>=rightx-0.3 && xxx <=rightx+0.3)
		{
		//	printf("cx=%f fx-0.3=%f fx+0.3=%f ",cx,f_x-0.3,f_x+0.3);
			//		printf("yy\n");
			if(zzz>=rightz-0.3 && zzz<=rightz+0.3)
			{
		//		printf("cz=%f fz-0.3=%f fz+0.3=%f ",cz,f_z-0.3,f_z+0.3);
				//			printf("in z\n");
				if((cy+0.2)>=righty-0.3 )
				{
		//			printf("here\n");
		//			printf("cy+0.2=%f f_y=%f\n",cy+0.2,f_y);
				//	printf("cy+0.2=%f fy-0.3=%f ",cy+0.2,f_y-0.3);
	...
	if(xxx>=leftx-0.3 && xxx <=leftx+0.3)
		{
		//	printf("cx=%f fx-0.3=%f fx+0.3=%f ",cx,f_x-0.3,f_x+0.3);
			//		printf("yy\n");
			if(zzz>=leftz-0.3 && zzz<=leftz+0.3)
			{
		//		printf("cz=%f fz-0.3=%f fz+0.3=%f ",cz,f_z-0.3,f_z+0.3);
				//			printf("in z\n");
				if((cy+0.2)>=lefty-0.3 )
	...
	if(xxx>=middlex-0.3 && xxx <=middlex+0.3)
		{
		//	printf("cx=%f fx-0.3=%f fx+0.3=%f ",cx,f_x-0.3,f_x+0.3);
			//		printf("yy\n");
			if(zzz>=middlez-0.3 && zzz<=middlez+0.3)
			{
		//		printf("cz=%f fz-0.3=%f fz+0.3=%f ",cz,f_z-0.3,f_z+0.3);
				//			printf("in z\n");
				if((cy+0.2)>=middley-0.3 )
				{
		//			printf("here\n");
		//			printf("cy+0.2=%f f_y=%f\n",cy+0.2,f_y);
				//	printf("cy+0.2=%f fy-0.3=%f ",cy+0.2,f_y-0.3);
	...
	if(xxx>=upx-0.3 && xxx <=upx+0.3)
		{
		//	printf("cx=%f fx-0.3=%f fx+0.3=%f ",cx,f_x-0.3,f_x+0.3);
			//		printf("yy\n");
			if(zzz>=upz-0.3 && zzz<=upz+0.3)
			{
		//		printf("cz=%f fz-0.3=%f fz+0.3=%f ",cz,f_z-0.3,f_z+0.3);
				//			printf("in z\n");
				if((cy+0.2)>=upy-0.3 )
	...
	if(xxx>=downx-0.3 && xxx <=downx+0.3)
		{
		//	printf("cx=%f fx-0.3=%f fx+0.3=%f ",cx,f_x-0.3,f_x+0.3);
			//		printf("yy\n");
			if(zzz>=downz-0.3 && zzz<=downz+0.3)
			{
		//		printf("cz=%f fz-0.3=%f fz+0.3=%f ",cz,f_z-0.3,f_z+0.3);
				//			printf("in z\n");
				if((cy+0.2)>=downy-0.3 )
	...
	if(xxx>=cornerx-0.3 && xxx <=cornerx+0.3)
		{
		//	printf("cx=%f fx-0.3=%f fx+0.3=%f ",cx,f_x-0.3,f_x+0.3);
			//		printf("yy\n");
			if(zzz>=cornerz-0.3 && zzz<=cornerz+0.3)
			{
		//		printf("cz=%f fz-0.3=%f fz+0.3=%f ",cz,f_z-0.3,f_z+0.3);
				//			printf("in z\n");
				if((cy+0.2)>=cornery-0.3 )
				{
		//			printf("here\n");
		//			printf("cy+0.2=%f f_y=%f\n",cy+0.2,f_y);
				//	printf("cy+0.2=%f fy-0.3=%f ",cy+0.2,f_y-0.3);
	...
	if(xxx>=forwardx-0.3 && xxx <=forwardx+0.3)
		{
		//	printf("cx=%f fx-0.3=%f fx+0.3=%f ",cx,f_x-0.3,f_x+0.3);
			//		printf("yy\n");
			if(zzz>=forwardz-0.3 && zzz<=forwardz+0.3)
			{
		//		printf("cz=%f fz-0.3=%f fz+0.3=%f ",cz,f_z-0.3,f_z+0.3);
				//			printf("in z\n");
				if((cy+0.2)>=forwardy-0.3 )
				{
		//			printf("here\n");
		//			printf("cy+0.2=%f f_y=%f\n",cy+0.2,f_y);
				//	printf("cy+0.2=%f fy-0.3=%f ",cy+0.2,f_y-0.3);
...
//	check_spot_light();
	...
	if(level==1)
	{
		check_teleportation_tile(cx,cy,cz);
		smp=check_swampy_tile(cx,cy,cz);
	}
...
//	glNormal3f(0.0,1.0f,0.0f);
	glTexCoord2f(0.0f,0.0f);
	glVertex3f(-100.0f ,-150.0f,140.0f);
	glTexCoord2f(1.0f,0.0f);
	glVertex3f(-100.f,150.0f,140.0f);
	glTexCoord2f(0.0f,1.0f);
	glVertex3f(-100.0f ,-150.0f,-5000.0f);
	glTexCoord2f(1.0f,1.0f);
	glVertex3f(-100.0f ,150.0f,-5000.0f);
	glEnd();
	glDisable(GL_TEXTURE_2D);
...
//	glNormal3f(0.0,1.0f,0.0f);
	glTexCoord2f(0.0f,0.0f);
	glVertex3f(120.0f ,-150.0f,140.0f);
	glTexCoord2f(1.0f,0.0f);
	glVertex3f(120.f,150.0f,140.0f);
	glTexCoord2f(0.0f,1.0f);
	glVertex3f(120.0f ,-150.0f,-5000.0f);
	glTexCoord2f(1.0f,1.0f);
	glVertex3f(120.0f ,150.0f,-5000.0f);
	glEnd();
	glDisable(GL_TEXTURE_2D);
...
/*	glEnable(GL_TEXTURE_2D);
	...
	glBegin(GL_QUADS);	
	glTexCoord2f(0.0f,0.0f);
	glVertex3f(-100.0f ,-150.0f,-400.0f);
	glTexCoord2f(1.0f,0.0f);
	glVertex3f(-100.f,150.0f,-400.0f);
	glTexCoord2f(0.0f,1.0f);
	glVertex3f(120.0f ,-150.0f,-400.0f);
	glTexCoord2f(1.0f,1.0f);
	glVertex3f(120.0f ,150.0f,-400.0f);
	glEnd();
	glDisable(GL_TEXTURE_2D);*/
...
void drawball()
{
		...
		glBlendFunc (GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
		glPushMatrix();
		glColor4f(1.0f, 0.1f, 1.0f, 0.5f);
		glTranslatef(leftx,lefty,leftz);
		glutSolidSphere(0.3,20,20); //0.1
		glPopMatrix();
		glDisable(GL_BLEND);
		...
		glPushMatrix();
		glColor4f(1.0f, 0.1f, 1.0f, 0.5f);
		glTranslatef(downx,downy,downz);
		glutSolidSphere(0.3,20,20); //0.1
		glPopMatrix();
		glDisable(GL_BLEND); 
		
		middley=middley+deltay;	
		glEnable (GL_BLEND);
		glBlendFunc (GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
		glPushMatrix();
		glColor4f(1.0f, 0.1f, 1.0f, 0.5f);
		...
		forwardy=forwardy+deltay;	
		glEnable (GL_BLEND);
		glBlendFunc (GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
		glPushMatrix();
		glColor4f(1.0f, 0.1f, 1.0f, 0.5f);
		glTranslatef(forwardx,forwardy,forwardz);
		glutSolidSphere(0.3,20,20); //0.1
		glPopMatrix();
		glDisable(GL_BLEND); 
...
void drawScene()
{
	...
	glTranslatef(tr,0.9,-0.1);
	glColor3f(0.5f,0.6f,0.0f);
	glutSolidCube(0.5);    //0.2
	glPopMatrix();	
	glPushMatrix();
	glTranslatef(te,0.9,-0.1);
	glColor3f(0.5f,0.6f,0.0f);
	glutSolidCube(0.5);    //0.2
	glPopMatrix();
...
if(level==4){
	...
	pp[1]='\0';
//	printf("%c\n",*pp);}
drawBitmapText(pp,7.0f,4.0f,-0.1f);
drawBitmapText("Time Elapsed:",5.0f,4.0f,-0.1f);
	}	

if(level==1){
drawBitmapText( "Level 1", 3.0f,4.0f,-0.1f);}
if(level==3){
drawBitmapText( "Level 3", 3.0f,4.0f,-0.1f);}
if(level==4){
drawBitmapText( "Level 4", 3.0f,4.0f,-0.1f);}
...
//	glutSolidSphere(0.1,20,20); //0.1
	...
	if(level==2){
	drawBitmapText("LEVEL 2",3.0f,4.0f,-0.1f);}
	if(level==4){
	drawBitmapText("LEVEL 4",3.0f,4.0f,-0.1f);
	pp[0]=r;
	pp[1]='\0';
